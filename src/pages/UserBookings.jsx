...
const UserBookings = () => {
  ...
  const onCancelBooking = async (bookingId) => {
    try {
      const res = await cancelBooking(bookingId);
      alert("La réservation a bien été annulée");
      onGetUserBookings(); // On rafraîchit pour que cela se mette bien à jour.
    } catch (e) {
      alert(e);
    }
  };
  ...
  return (
    <div>
      ...
      <h1 className="user-bookings">Mes réservations</h1>
      <div className="list-bookings">
        {/* On fait une boucle pour afficher toutes les résas une par une grâce à map. Quand on crée un map il faut donner une clé unique à chaque élément de la liste */}
        {bookings.map((booking, index) => (
          <Card
            className="card-user-booking"
            key={"bookings" + index}
          ...
          >
            <Card.Body>
              ...
              {booking.status === "canceled" && (
                <Card.Text style={{ color: "red" }}>Annulée par vous</Card.Text>
              )}
              {(booking.status === "pending" ||
                booking.status === "accepted") && (
                <Button
              ...
              {booking.status === "accepted" && booking.resto && (
                <Button
                  variant="primary"
                  onClick={() => navigate("/ratingpage/" + booking.resto._id)}
                >
